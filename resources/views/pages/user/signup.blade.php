          ...
          <div class="col-12 col-lg-12 col-xl-12 mx-auto">
            ...
            @endif
              ...
              <div class="form-check custom-control custom-checkbox">
                <input id="customer" name="customer" class="form-check-input" type="radio"  value="Customer" required>
                <label class="custom-control-label" for="remember-me">{{trans('words.customer')}}</label>                
              </div>
              </div>

               <div class="col">
              <div class="form-check custom-control custom-checkbox">
                <input id="vendor" name="customer" class="form-check-input" type="radio" value="Vendor" required>
                <label class="custom-control-label" for="remember-me">{{trans('words.vendor')}}</label>                
              </div>
              </div>
            </div>
              <div class="row mt-4">
              <div class="col">
              <div class="form-check custom-control custom-checkbox">
                <input id="agree_policy" name="agree_policy" class="form-check-input" type="checkbox" required>
                <label class="custom-control-label" for="remember-me">{{trans('words.agree_policy')}}</label>                
              </div>
              </div>
            </div>


            <button class="btn-submit btn-block mb-4" type="submit">{{trans('words.sign_up')}}</button>
            {!! Form::close() !!}
            <p class="text-3 text-center mb-3">{{trans('words.already_sign_up')}} <a class="btn-link" href="{{ url('login') }}" title="login">{{trans('words.login_text')}}</a></p>
