...
class LicenseBoxAPI
{
	...
	{
		$this->product_id = '0EE13A69';
		$this->api_url = 'https://license.viavilab.com/';
		$this->api_key = 'B2FEDAEF0F7302366227';
	...
	private function call_api($method, $url, $data)
	{
		...
		curl_setopt(
			...
			CURLOPT_HTTPHEADER,
			array(
				'Content-Type: application/json', 'LB-API-KEY: ' . $this->api_key, 'LB-URL: ' . $this_url, 'LB-IP: ' . $this_ip, 'LB-LANG: ' . $this->api_language
			)
		);
		...
		if (!$result && !LB_API_DEBUG) {
			$rs = array('status' => FALSE, 'message' => LB_TEXT_CONNECTION_FAILED);
		...
		if ($http_status != 200) {
			if (LB_API_DEBUG) {
				...
				$rs = array('status' => FALSE, 'message' => LB_TEXT_INVALID_RESPONSE);
				return json_encode($rs);
			}
		}
		curl_close($curl);
	...
	{
		$data_array = array();
		$get_data = $this->call_api('POST', $this->api_url . 'api/check_connection_ext', json_encode($data_array));
	...
	{
		file_put_contents($this->license_file, 'licfile-text', LOCK_EX);
		return array('status' => TRUE, 'client' => 'client' ,'message' => 'Valid license // Please ping me for more scripts in low cost https://wa.me/919822491553');
		$data_array = array(
			"product_id" => $this->product_id, "license_code" => $license, "client_name" => $client, "verify_type" => $this->verify_type
		);
	...
	public function verify_license($time_based_check = false, $license = false, $client = false)
	{
		...
		if (!empty($license) && !empty($client)) {
			...
			if (is_file($this->license_file)) {
				$data_array = array(
					"product_id" => $this->product_id, "license_file" => file_get_contents($this->license_file), "license_code" => null,
					"client_name" => null
				);
			} else {
				$data_array = array();
		...
		if ($time_based_check && $this->verification_period > 0) {
			...
			$today = date('d-m-Y');
			if (empty($_SESSION["451bb9cadc8876c"])) {
				$_SESSION["451bb9cadc8876c"] = '00-00-0000';
			}
			if ($type == 1) {
				$type_text = '1 day';
			} elseif ($type == 3) {
			...
			if (strtotime($today) >= strtotime($_SESSION["451bb9cadc8876c"])) {
	...
	public function download_update($update_id, $type, $version, $license = false, $client = false, $db_for_import = false)
	{
		...
		curl_setopt(
			$ch,
			CURLOPT_HTTPHEADER,
			array('LB-API-KEY: ' . $this->api_key, 'LB-URL: ' . $this_url, 'LB-IP: ' . $this_ip, 'LB-LANG: ' . $this->api_language)
		);
		if (LB_SHOW_UPDATE_PROGRESS) {
