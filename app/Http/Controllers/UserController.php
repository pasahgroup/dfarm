...
class UserController extends Controller
{
        ...
        public function payment()
    {
        ...
        $transactions_list = Transactions::where('user_id',$user_id)->orderBy('id','DESC')->paginate(10);
         ///dd($transactions_list);
        return view('pages.user.payment',compact('user','transactions_list'));
    }
    ...
    //     if (!session()->has('session_number')) {
    // session(['session_number' => rand(100000, 999999)]);
    //  }

       if (!session()->has('session_number')) {
        session(['session_number' => rand(10000000, 999999999)]);
    }
...
// $session_no = session('session_number');
...
//             $payment_trans->gateway = '';
//             $payment_trans->usd = $plan_amount;
...
//  Session::put('transaction_id',$transaction_id);
        ...
        return view('pages.payment.payment_method',compact('plan_info','discount_price_less','plan_id','coupon_percentage','coupon_code'));
    }
